...
import { 
  ...
  ExternalLink, PackagePlus, CheckCircle, XCircle
} from "lucide-react";
...
export default function Suppliers() {
  ...
  const handleUpdateOrderStatus = async (id, inventory_status) => {
    try {
      await API.put(`/inv-orders/${id}`, { inventory_status });
      setOrders((prev) => prev.map((order) => order.id === id ? { ...order, inventory_status } : order));
      fetchData();
    } catch (err) {
                    ...
                    <div className="flex flex-col gap-2 min-w-0">
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                          ...
                          <span className="font-medium truncate">Lead Time: {s.lead_time_days ?? 0} days</span>
                        </div>
                      </div>
                    </div>
                ...
                {orders.map((o) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <StatusBadge status={o.inventory_status === "Available" ? "Active" : o.inventory_status} />
                    </div>
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in slide-in-from-bottom sm:slide-in-from-bottom-8 duration-300">
            ...
            <form onSubmit={handleSubmit} className="px-6 sm:px-8 py-6 sm:py-8 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Lead Time (days)</label>
